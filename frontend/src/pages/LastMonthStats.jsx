...
import { AreaChart, Brush, ReferenceLine, Area, Rectangle, Pie, PieChart, BarChart, Bar, Cell, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer } from 'recharts';
...
const LastMonthStats = ({storeName, setLoadingByDateRange }) => {

  const { fetchDataByDateRange } = useDataByDateRange();
  const {lastMonthData, reverseLastMonthData, lastMonthCustomerData , customerColors} = Data()
  ...
  const changeData = lastMonthData.map((entry,index)=>{
      const previousEntry = index > 0 ? lastMonthData[index-1] : null;
        ...
        <div className='charts'>
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Legend />
              <Bar dataKey="ManCount" stackId="1" fill="#87CEEB" activeBar={<Rectangle fill="#6A93C3" stroke="#FFD700" />} />
              <Bar dataKey="WomanCount" stackId="1" fill="#FFC0CB" activeBar={<Rectangle fill="#C4878E" stroke="#FFD700" />} />
              <Bar dataKey="KidCount" stackId="1" fill="#ffb90f" activeBar={<Rectangle fill="#ee9a00" stroke="#FFD700" />} />
            </BarChart>
          </ResponsiveContainer>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart
        ...
        <div className='charts-three'>
          <ResponsiveContainer width="100%" height="100%">
            <PieChart width={400} height={400}>
              ...
              >
                {lastMonthCustomerData.map((entry, index) => (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <PieChartActiveShape data={lastMonthData} />
          </ResponsiveContainer>

        </div>
        <div className='main-title' style={{marginTop:50}}>
          <h2>LAST MONTH RECORDS</h2>
        </div>
        <RecordTable data={reverseLastMonthData}/>
      </div>
    </main>
  )
}
